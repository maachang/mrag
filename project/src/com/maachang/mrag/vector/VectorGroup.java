...
public class VectorGroup {
    ...
    public VectorGroup(String group, String path, String name,
        long time, VectorChunk[] docs, Queue<VectorChunk> cman) {
        this.group = group;
        this.filePath = path;
        this.fileName = name;
        this.fileTime = time;
    ...
    // 得点を計算: コサイン類似度.
    private static final double score(double[] a, double[] b) {
        double d = 0, na = 0, nb = 0, av, bv;
        int i, len = a.length;
        for(i = 0; i < len; i ++) {
            av = a[i]; bv = b[i];
            d += av * bv;
        ...
        return d / (Math.sqrt(na * nb) + 1.0E-8);
    }
    ...
    public int search(VectorChunk[] out, double[] queryEmbedding) {
        // 単純検索.
        int i;
    ...
    protected VectorChunk[] getDocuments() {
        return documents;
    }
