...
public class NetBuffer {
	...
	protected static class Entry {
		...
		// ntry内の読み込み処理.
		//  update 読み込み結果を更新する場合は true.
		//  out 読み込み内容を格納するバイナリ.
		//  off 読み込みバイナリのオフセット値を設定します.
		//  len 読み込みバイナリの長さを設定します.
		// 戻り値: 読み込まれた長さが返却されます.
	...
	protected void closeCheck() {
		if(closeFlag) {
			throw new MRagException("It's already closed.");
		}
	}
	...
	// 指定位置までの情報を取得.
	//  pos 指定位置を取得します.
	// 戻り値: null の場合は指定位置の条件の取得に失敗しました.
	...
	// スキップ.
	//  len スキップするデータ長を設定します.
	// 戻り値: 実際にスキップされた数が返却されます.
	...
	// データ取得.
	// 戻り値: 設定されているデータを全て取得します.
	public byte[] toByteArray() {
		return toByteArray(false);
	}

	// データ取得.
	//  readByClear true の場合取得したデータは削除されます.
	// 戻り値: 設定されているデータを全て取得します.
	public byte[] toByteArray(boolean readByClear) {
		...
		if(len != allLength) {
			throw new MRagException("The planned length (" + allLength
	...
	// 現在の内容をOutputStreamに出力.
	//  o 対象のOutputStreamを設定します.
	//    このOutputStreamはクローズしません.
	public void outputStream(OutputStream o) throws IOException {
		closeCheck();
		// firstEntry.
		Entry entry = firstEntry;
		// Entryが無くなるまでループ.
		while(entry != null) {
			// 書き込み処理.
			o.write(entry.value, entry.readPosition, entry.readRemaining());
			// 次のEntry.
			entry = entry.next;
		}
		// 情報を全クリア.
	...
	// 戻り値: InputStreamが返却されます.
	public InputStream getInputStream() {
		return new NetBufferByInputStream(this);
	}

	// OutputStreamオブジェクトを取得.
	// 戻り値: OutputStreamが返却されます.
	public OutputStream getOutputStream() {
		return new NetBufferByOutputStream(this);
	}

	// NetBuffer用のInputStream.
	private static final class NetBufferByInputStream extends InputStream {
		private final byte[] b1 = new byte[1];
		private NetBuffer buffer;
		protected NetBufferByInputStream(NetBuffer buffer) {
			if(buffer == null) {
	...
	// NetBuffer用のOutputStream.
	private static final class NetBufferByOutputStream extends OutputStream {
		private final byte[] b1 = new byte[1];
		private NetBuffer buffer;

		public NetBufferByOutputStream(NetBuffer buffer) {
			if(buffer == null) {
				throw new NullPointerException();
			}
			this.buffer = buffer;
